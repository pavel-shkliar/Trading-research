...
- [ ] n — независимые эпизоды (через `collapse_to_episodes`), не сырые
      пересекающиеся дни
- [ ] Порог/параметры гипотезы зафиксированы ДО просмотра результата
      на новых/будущих данных — не подгонялись задним числом
- [ ] Указано, какая часть истории in-sample (использована для поиска),
      а какая — настоящий out-of-sample (данные, которых не существовало
      на момент формулировки/заморозки параметров)
...
Эти параметры **не меняем** больше, что бы ни показали новые данные.
Раз в месяц-два, по мере накопления новых дней через ежедневный автосбор,
прогоняем ТЕ ЖЕ САМЫЕ, замороженные тесты на данных, которых на момент
заморозки не существовало. Это медленно, но единственный доступный нам
честный способ получить настоящий, слепой out-of-sample результат.
