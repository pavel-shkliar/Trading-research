...
**Статус**: понижен — работает на BTC, НЕ подтверждается на 17 монетах
(см. проверку в конце раздела). Не обобщаем как универсальную
закономерность (обновлено 2026-09-07)
...
**Понижение статуса после проверки на 17 монетах (2026-09-07,
`backtest_h1b_multi_symbol.py`, горизонт 90 дней)**: скачали историю по
топ-20 крипто-перпетуалов Binance (исключив токенизированные
традиционные активы вроде золота/акций и монеты младше ~400 дней —
осталось 17). Только **8 из 17** монет показывают положительный эдж,
сам BTC (+6.87) скромнее нескольких других (SOL +28.29, BNB +22.67).

Объединённый пул (969 эпизодов по всем 17 монетам, избыточная доходность
каждого эпизода относительно СВОЕЙ базы монеты): средняя избыточная
доходность **−6.51%**, 95% ДИ [−14.40%, +1.38%] пересекает ноль,
**t-test p=0.1065 — не значимо**.

**Вывод**: H1b, вероятно, НЕ универсальная закономерность деривативного
рынка, а BTC/крупнокапитализационная особенность (или шум на конкретных
монетах) — статус понижен с "многообещающая" до "работает на BTC, не
подтверждается на широком крипторынке, обобщать нельзя". DOGE дал
экстремально отрицательный эдж (−50.80) — вероятно, единичный
искажающий период, отдельно не разбирали, не считаем установленным
фактом.
