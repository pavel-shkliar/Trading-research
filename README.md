...
generate_charts.py           Reproduces the two charts in this README from live data

downloaders/                 Incremental downloaders
  download_*.py                candles, funding rate, open interest,
                                long/short ratio, taker volume, DVOL, spot candles
  download_all.py              runs all of the above for one symbol
...
charts/                      PNGs embedded above, regenerated by generate_charts.py
...
python downloaders/download_all.py --days 2600
python compute_indicators.py
python backtests/backtest_h6b_walkforward.py   # the strongest result
python generate_charts.py                 # reproduce the README charts
```
